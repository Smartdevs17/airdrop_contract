...
const {  generateMerkleRootFromArray, generateMerkleProofWithRoot, getUserDataFromCSV } = merkle;
...
describe('MerkleAirdrop', function () {
  ...
  it('Should reject invalid claims', async function () {
    ...
    const amount = ethers.parseEther("100.0").toString();
    const proof = await generateMerkleProofWithRoot(hash, user7.address, amount);
  ...
  it('Should handle invalid proofs correctly', async function () {
    const { merkleAirdrop, user2, hash} = await loadFixture(deployMerkleAirdropFixture);
    // Assuming user1 has an invalid proof
    const amount = ethers.parseEther("1.0").toString();
    const proof = await generateMerkleProofWithRoot(hash, user2.address, amount);
